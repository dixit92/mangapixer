...
```powershell
msiexec /i MangaPixer-1.21.0-windows-x64.msi /qn                 # silent, defaults
msiexec /i MangaPixer-1.21.0-windows-x64.msi /qn RUNATSIGNIN=0   # silent, without start at sign-in
```
...
- **Uninstall:** Windows Settings > Apps > **MangaPixer** > Uninstall, or `msiexec /x MangaPixer-1.21.0-windows-x64.msi /qn`. This removes the program folder, the Start menu shortcut and the start-at-sign-in entry. **Your data in `%LOCALAPPDATA%\MangaPixer` is kept.** Delete that folder yourself if you want everything gone.
