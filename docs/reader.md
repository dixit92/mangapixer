...
Browsers only offer WebGPU to secure pages, so on a phone or tablet that opens MangaPixer as `http://192.168.x.x:…` (a LAN install, Docker or Unraid without a reverse proxy, or the Windows app with **Allow LAN access**), Enhance runs on WebGL2 instead. It looks the same as WebGPU Enhance with Efficient quality; only **Max quality** needs WebGPU. For WebGPU, open MangaPixer over HTTPS (see [Reverse proxy and HTTPS](reverse-proxy-and-https.md)).
