...
public sealed class ServerProcessManager : IAsyncDisposable
{
    ...
    /// Test-only hook: MangaPixer.Server.exe itself never takes arguments (it
        ...
        lock (_lock)
        {
            ...
            if (!File.Exists(_serverExecutablePath))
                throw new FileNotFoundException(
                    "MangaPixer.Server.exe was not found at the expected distribution path (server\\ under the tray's folder, or next to the tray executable).",
