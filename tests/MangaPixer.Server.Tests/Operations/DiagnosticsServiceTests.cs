...
public sealed class DiagnosticsServiceTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "mangaplex-diag-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    private async Task<(MangaPlexDbContext db, DiagnosticsService service)> SetupAsync()
    {
