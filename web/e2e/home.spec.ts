...
/**
 * Full-stack smoke tests against a running MangaPixer instance (see baseURL in
...
test('unauthenticated visit redirects to the login screen', async ({ page }) => {
  ...
  await expect(page.getByText('MangaPixer Login')).toBeVisible();
});
...
test('admin can log in and reach the app', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'MangaPixer' })).toBeVisible();
});
