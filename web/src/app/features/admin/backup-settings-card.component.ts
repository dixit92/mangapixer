 ...
 * of the database is written. Saving never moves or deletes existing snapshots.
 *
...
@Component({
  ...
  standalone: true,
  imports: [MatButtonModule, MatCardModule, MatCheckboxModule, MatIconModule, MatRadioModule],
    ...
    <mat-card>
      ...
      <mat-card-content>
        @if (loading()) {
          ...
          <mat-radio-group
            [value]="draftMode()"
            [disabled]="!s.locationChangeAllowed || busy()"
  ...
  private send(request: UpdateBackupSettingsRequest, validateOnly: boolean): void {
    ...
    this.api.updateBackupSettings(request).subscribe({
      next: (result) => {
        ...
        this.movedNotice.set(result.locationChanged);
        this.changed.emit();
      },
      error: (err: ApiError) => {
        this.busy.set(false);
